...
#define HOP_SCAN_DWELL_MS 70
#define HOP_RETRY_DELAY_MS 250
...
static bool scan_channel_for_target(uint8_t channel, wifi_ap_record_t *best)
{
    ...
    scan.scan_time.active.max = HOP_SCAN_DWELL_MS + 20;
...
static void hop_task(void *arg)
{
    ...
    for (;;) {
        ...
        size_t count = s_hop_mode == HOP_MODE_FULL ? HOP_FULL_CHANNEL_COUNT : HOP_COMMON_CHANNEL_COUNT;
        ...
        if (!connected_attempt && !s_sta_connected) {
            vTaskDelay(pdMS_TO_TICKS(HOP_RETRY_DELAY_MS));
            s_hop_pending = true;
